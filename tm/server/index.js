...
app.post('/tasklists', async (request, response) => {
  ...
  const { tasklistType } = request.body;
  tasklists.push({
    tasklistName,
    tasklistType,
    tasks: []
...
// Удаление списка
app.delete('/tasklists/:tasklistId', async (request, response) => {
  const tasklistId = Number(request.params.tasklistId);
  //const taskId = Number(request.params.taskId);
  ...
  if (tasklistId < 0 || tasklistId >= tasklists.length) {
    response
      ...
      .json({
        info: `There is no tasklist with id = ${
          tasklistId} or task with id =`
      });
    return;
  }

  //const deletedTaskName = tasklists[tasklistId].tasks[taskId];
  tasklists.splice(tasklistId, 1);
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
    .json({
      info: `Task  was successfully deleted from tasklist `
    });
});
...
app.listen(port, hostname, async (err) => {
  ...
  tasklistsFromFile.forEach(({ tasklistName, tasklistType, tasks }) => {
    tasklists.push({
      tasklistName,
      tasklistType,
      tasks: [...tasks]
