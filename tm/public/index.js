...
class AppModel {
  ...
  static async addTasklist(tasklistName, tasklistType) {
    console.log(JSON.stringify({ tasklistName , tasklistType}));
    const result = await fetch(
      'http://localhost:4321/tasklists',
      {
        ...
        body: JSON.stringify({ tasklistName , tasklistType})
      }
    );
...
class App {
  ...
  onEscapeKeydown = ({ key }) => {
    if (key === 'Escape') {
      ...
      const type = document.getElementById('tm-tasklist-type')
  ...
  onInputKeydown = async ({ key, target }) => {
    if (key === 'Enter') {
      if (target.value) {
        const type = document.getElementById('tm-tasklist-type')
        await AppModel.addTasklist(target.value, type.value);
      ...
      document.getElementById('tm-tasklist-type').style.display = 'none';

      document.getElementById('tm-tasklist-add-tasklist').style.display = 'inherit';
    }
  };
  ...
  deleteTasklist = async ({ tasklistID}) => {
    try {
      ...
      const Id = Number(tasklistID.split('TL')[1]);
      this.tasklists.slice(Id, 1);
  ...
  async init() {
    ...
    //
    tasklists.forEach(({ tasklistName, tasklistType, tasks }) => {
    ...
    document.getElementById('tm-tasklist-add-tasklist')
      .addEventListener(
        ...
        (event) => {
          ...
          const input1 = document.getElementById('tm-tasklist-type');
...
class Tasklist {
  constructor({
    ...
    this.deletelist = deleteTasklist;
  }

  onDeleteTasklistButtonClick = async (taskID) => {
    //const taskIndex = Number(taskID.split('-T')[1]);
    //const taskName = this.tasks[taskIndex];

    if (!confirm(` '${this.tlName}' будет удален. Продолжить?`)) return;
    ...
    try {
      ...
      //   //taskId: taskIndex
      // });
      //document.getElementById(`${this.tlID}`).remove();
      this.deleteTasklist();
    } catch (error) {
      console.error('ERROR', error);
    }
  ...
  onAddTaskButtonClick = async () => {
    const newTaskName = prompt('Введите остановку:');
  ...
  onEditTask = async (taskID) => {
    ...
    const newTaskName = prompt('Введите новую остановку', oldTaskName);
  ...
  onDeleteTaskButtonClick = async (taskID) => {
    ...
    if (!confirm(` '${taskName}' будет удалена. Продолжить?`)) return;
  ...
  render() {
    ...
    const type = document.createElement('header');
 
    header.classList.add('tm-tasklist-header');
    header.innerHTML = this.tlName;
    tasklist.appendChild(header);

    type.classList.add('tm-tasklist-header');
    type.innerHTML = this.tlType;
    tasklist.appendChild(type);
    ...
    del_button.classList.add('tm-tasklist-del-tasklist');
    button.innerHTML = 'Добавить остановку';
    del_button.innerHTML = 'Удалить маршрут';
    button.addEventListener('click', this.onAddTaskButtonClick);
    del_button.addEventListener('click', this.onDeleteTasklistButtonClick);
    footer.appendChild(button);
    footer.appendChild(del_button);
  ...
  rerender(){
    document.body.classList.remove('tm-tasklist')
  }
}
